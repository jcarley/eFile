...
Vagrant.configure("2") do |config|
  ...
  @application_name = 'efile'
  @virtual_machine = "ffsdevweb-#{@application_name}"
  ...
  config.vm.define @virtual_machine do |config|

    box_name = @virtual_machine
    host_name = "local.#{box_name}.com"

    config.vm.provider :virtualbox do |vb|
      vb.name = @application_name
